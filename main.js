...
// 💣 2. The Personas including Auto-Agent
...
function updateLongTermMemory(userText) {
    ...
    if(userText.includes("ناڤێ من") || userText.includes("حەز دکەم") || userText.includes("ئەز ")) {
        memory += userText + " | ";
        if(memory.length > 1000) memory = memory.substring(memory.length - 1000);
...
const GROQ_API_KEY = _0xpart1 + _0xpart2 + _0xpart3;
    ...
    if (!response.ok) throw new Error(`Groq API Error: ${(await response.json()).error?.message}`);
